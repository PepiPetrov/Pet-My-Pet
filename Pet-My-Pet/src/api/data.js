...
export async function getArticles() {
    const teams = await api.get(host + '/jsonstore/pets');
    return teams;
}
...
export async function createAritcle(article) {
    article.likes=0
    const result = await api.post(host + '/jsonstore/pets', article);
    
    return result;
}

export async function editArticle(id, article) {
    return await api.put(host + '/jsonstore/pets/' + id, article);
}
...
export async function filterByCategory(category){
    const data=await api.get(`http://localhost:3030/jsonstore/pets/`)
